...
Dictionary learning has grown as a field, from learning either the dictionary, or the coefficients, to now learning both the dictionary and the coefficients. It is being used for denoising [ref], inpainting~\citep{mairal2009online}, and classification [ref]. Even then, the utility of extracting \emph{meaningful} representations has been somewhat limited. The representation itself did not matter as long as the classification or reconstruction error was low. However, this is exactly the case, when it comes to neural signals. Here, the fidelity of the learned representation is important as the shape is crucial in many clinical applications [ref].

[patch based to convolutional]

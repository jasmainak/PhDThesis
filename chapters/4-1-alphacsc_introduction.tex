...
Neural time series data, either non-invasive such as electroencephalograhy (EEG) 
...
Recently, there have been several attempts for extracting more realistic and precise morphologies directly from unfiltered electrophysiology signals, via dictionary learning approaches \cite{jost2006motif,brockmeier2016learning,hitziger2017adaptive,gips2017discovering}.
%
...
%
More recently, the `sliding window matching' (SWM) algorithm \cite{gips2017discovering} was proposed for learning time-varying atoms by using a correlation-based approach that aims to identify the recurring patterns. Even though some success has been reported with these algorithms,
...
A natural way to cast the problem of learning a dictionary of shift-invariant atoms into an optimization problem is a convolutional sparse coding (CSC) approach~\cite{Grosse-etal:2007}. 
